...
def get_file_names(file_name_base, number_type=None, sort_reverse=False, verbose=True):
    ...
    -------
    path_file_names : str
        name[s] of file[s], with full path
    [file_numbers] : int[s] and/or float[s]
        number in file name
    '''
    from gizmo_analysis import gizmo_default
...
def get_file_names_intersect(
    file_name_bases=[], numbers_in=[], number_type=float, sort_kind=None, arrayize=False
):
    '''
    ...
    Parameters
    ----------
    file_name_bases: str or list
        base name[s] of files, with full path, using * as wildcard
    numbers_in : float or list
        number[s] (such as scale-factors)
    number_type : dtype
        what kind of number in file name to get. options: None, int, float
    sort_kind : str
        how to sort file names/numbers: None, 'forward', 'reverse'
    arrayize : bool
    ...
    if numbers_in is None or numbers_in == []:
        ...
        file_numbers = get_file_names_nearest_number(
            file_name_bases[0], numbers_in, number_type, arrayize=arrayize
        )[1]
    ...
    if dict_or_array_to_write is not None:
        ...
        with h5py.File(file_name_base, 'w') as file_out:
            ...
            for key in dict_out:
                ...
                Say.say(
                    f'  {key_use} | {dict_out[key].dtype}, shape = {dict_out[key].shape}', verbose,
                )
            print()
