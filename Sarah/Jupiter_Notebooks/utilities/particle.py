...
def get_velocity_dispersion_v_distance(
    ...
    part_weights = None
    if weight_by_mass:
        part_weights = part_spec.prop('mass', part_indices)
...
def get_halo_properties(
    ...
    for spec_i, spec_name in enumerate(species):
        ...
        mass_cum_in_bins += np.sum(part[spec_name]['mass'][distance_indices]) + np.cumsum(
            mass_in_bins
        )
...
def get_galaxy_properties(
    ...
    def _get_galaxy_radius_mass_indices(
        ...
        mass_in_bins = DistanceBin.get_histogram(distances, masses)
